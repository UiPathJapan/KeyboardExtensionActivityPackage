...
namespace UiPathTeam
{
    namespace KeyboardExtension
    {
        ...
        private:
            ...
            void UninstallCallWndProcHook(std::map<DWORD, HHOOK>::iterator&);
            ...
            HHOOK m_hKeybdHook;
            HHOOK m_hMouseHook;
            std::map<DWORD, HHOOK> m_CallWndProcHookMap;
            ATOM m_atomWndClass;
            HWND m_hwndMessage;
        ...
        inline bool Server::OnKeybdHook(UINT uMsg, const KBDLLHOOKSTRUCT* pKHS)
        {
            ...
            if ((pKHS->flags & LLKHF_UP) == 0)
            {
                ...
                if (!m_pIpcBlock->m_ToggleSequece.IsZero() && m_pressedKeys.IsSet(m_pIpcBlock->m_ToggleSequece))
                {
                    DBGPUT(L"Toggle key sequence detected.");
                    if (m_pIpcBlock->CanChange())
                    {
                        m_pIpcBlock->ToggleFlags();
                        DBGPUT(L"m_pIpcBlock->m_dwFlags: current=%08lx last=%08lx", m_pIpcBlock->m_dwFlags, m_pIpcBlock->m_dwFlagsToggled);
                        NotifyAgents();
                    }
                }
            }
            ...
            if ((pKHS->flags & LLKHF_UP) == 0 && (m_pIpcBlock->m_dwFlags & FLAG_DISABLE_KEYBD) == FLAG_DISABLE_KEYBD)
            {
                //DBGPUT(L"Blocked.");
                return true;
            }
        ...
        inline bool Server::OnMouseHook(UINT uMsg, const MSLLHOOKSTRUCT* pMHS)
        {
            ...
            if (m_pIpcBlock != NULL)
            {
                if ((m_pIpcBlock->m_dwFlags & FLAG_DISABLE_MOUSE) == FLAG_DISABLE_MOUSE)
                {
                    //DBGPUT(L"Blocked.");
                    return true;
                }
